...
if ($_POST['Rs'] == 'ok') {
  ...
  while ($row = $query->fetch()) {
    ...
    echo '
      ...
      <div class="accordion accordion-flush p-0" id="accordion' . $row['id_linea'] . '">
        <div class="accordion-item mb-3 rounded border">
          <h2 class="accordion-header" id="flush-heading' . $row['id_linea'] . '">
            <button class="accordion-button collapsed m-0 p-0" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapse' . $row['id_linea'] . '" aria-expanded="false" aria-controls="flush-collapse' . $row['id_linea'] . '">
          ...
          <div id="flush-collapse' . $row['id_linea'] . '" class="accordion-collapse collapse" aria-labelledby="flush-heading' . $row['id_linea'] . '" data-bs-parent="#accordion' . $row['id_linea'] . '">
            <div class="accordion-body vh-20 small p-1">' . $row['info_linea'] . '</div>
          </div>
        </div>
      </div>
