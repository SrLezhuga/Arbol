...
try {
  ...
  if ($result->status === 200) {
    ...
    for ($i = 1; $i < $countmax + 1; $i++) {
      ...
      foreach ($arr_json['parts'] as $t_items) {


        if (isset($t_items['piesItem']['digitalAssets']['imageURL800'])) {
          $img = $t_items['piesItem']['digitalAssets']['imageURL800'];
        } else if (isset($t_items['piesItem']['digitalAssets'][0]['imageURL800'])) {
          $img = $t_items['piesItem']['digitalAssets'][0]['imageURL800'];
        } else if (isset($t_items['piesItem']['digitalAssets'][1]['imageURL800'])) {
          $img = $t_items['piesItem']['digitalAssets'][1]['imageURL800'];
        } else {
        ...
        $items[] = array(
          'count' => $count++,
          'partNumber' => $t_items['piesItem']['partNumber'],
          'brandCode' => $t_items['piesItem']['brandCode'],
          'brandName' => $t_items['piesItem']['brandName'],
          'logo' => (isset($t_items['piesItem']['brandAdditionalInfo']['logoImageURL100x40'])) ? $t_items['piesItem']['brandAdditionalInfo']['logoImageURL100x40'] : 'assets/media/img/loader/PlaceholderCatalogo.png',
          'imageURL' => $img
        );
      }
    }
